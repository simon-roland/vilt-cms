...
class CmsInstallCommand extends Command
{
    ...
    private function isFreshInstall(): bool
    {
        ...
        // Laravel <=11: app.js has `import './bootstrap'`
        // Laravel 12+:  app.js is a near-empty stub (`//`)
        // Either way, a stock file has no framework bootstrap (no Inertia, no Vue).
        $stockAppJs = $appJsExists
            && ! $appTsExists
            && ! str_contains($appJsContent, 'createInertiaApp')
            && ! str_contains($appJsContent, 'createApp');
